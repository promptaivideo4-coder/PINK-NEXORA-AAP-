...
/**
 * NOTE: the hardcoded demo payroll records were removed in the final
 * release audit — this screen only shows payroll data generated by the
 * Payroll & Earnings screen (device cache) and never fake people.
 */
...
export default function PayrollBreakdown({ navigate }: NavigationProps) {
  const [record, setRecord] = useState<PayrollRecord | null>(() => {
    const records = readJson<PayrollRecord[]>(PAYROLL_KEY, [] as PayrollRecord[]);
  ...
  return (
    <div className="min-h-screen bg-[#fcf9f8] text-on-background antialiased print:bg-white">
      ...
      <main className="mx-auto w-full max-w-4xl px-4 pb-28 pt-24 sm:px-6 print:max-w-none print:px-0 print:pt-0">
        ...
        <section className="no-print flex flex-col gap-2 sm:flex-row"><button type="button" onClick={exportPdf} className="flex-1 rounded-xl border border-[#e0bec6] bg-white px-4 py-3 text-xs font-bold text-primary hover:bg-[#fde7f3]"><Download className="mr-1 inline h-4 w-4" /> Export PDF Statement</button>{!isSettled && <div className="flex flex-col sm:flex-row gap-2 sm:items-center flex-1"><button type="button" onClick={() => setSettleConfirm(true)} className="rounded-xl bg-primary px-4 py-3 text-xs font-bold text-white hover:opacity-90">Mark as Settled (device record)</button><p className="text-[10px] text-on-surface-variant leading-snug sm:mt-0">Canonical payout settlement runs server-side (daily 10 PM IST job) — this button only records local acknowledgment.</p></div>}</section>
      </main>
