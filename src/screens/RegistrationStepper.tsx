...
import { X, ChevronDown, ArrowRight, ArrowLeft, AlertCircle, Check, MapPin } from 'lucide-react';
...
export default function RegistrationStepper({ navigate }: NavigationProps) {
  ...
  const [confirmedLocation, setConfirmedLocation] = useState<ConfirmedShopLocation | null>(null);

  // Step 1 → validate → Step 2 (location)
  ...
  // Step 2 → confirm & create account (location MANDATORY)
  const handleComplete = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);

    // VALIDATION: location must be set + confirmed
    if (!confirmedLocation) {
      setError('Shop location is required. Please set your exact shop location on the map to continue.');
      return;
    }
    const { latitude, longitude } = confirmedLocation;
    if (
    ...
    try {
      ...
      if (data.session) {
        // Create shop workspace (draft), phir canonical location save
        try {
        ...
        try {
          await updateShopLocation(supabase, {
            latitude: confirmedLocation.latitude,
            longitude: confirmedLocation.longitude,
            address: confirmedLocation.address || null,
            city: confirmedLocation.city || null,
            area: confirmedLocation.area || null,
  ...
  return (
    <div className="bg-background text-on-background min-h-screen flex flex-col selection:bg-primary-container selection:text-white">
      ...
      <main className="flex-1 mt-16 px-4 pt-8 pb-[150px] max-w-md mx-auto w-full flex flex-col">
        {/* Stepper Header */}
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2 relative">
            ...
            <div className="flex flex-col items-center bg-background px-1">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[13px] font-medium mb-1 shadow-sm ${step >= 1 ? 'bg-primary-container text-white' : 'bg-surface-variant text-on-surface-variant'}`}>1</div>
              <span className={`text-[13px] font-semibold ${step === 1 ? 'text-primary' : 'text-on-surface-variant'}`}>Info</span>
            </div>

            <div className="flex flex-col items-center bg-background px-1">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[13px] font-medium mb-1 shadow-sm ${step === 2 ? 'bg-primary-container text-white' : 'bg-surface-variant text-on-surface-variant'}`}>
                {confirmedLocation ? <Check className="w-4 h-4" /> : '2'}
              </div>
              <span className={`text-[13px] font-semibold ${step === 2 ? 'text-primary' : 'text-on-surface-variant'}`}>Location</span>
            </div>
          </div>
        ...
        {/* Error */}
        {error && (
          <div className="mb-4 p-3 rounded-xl bg-error/10 border border-error/20 flex items-center gap-3 text-error text-sm font-medium">
        ...
        {step === 1 ? (
            ...
            >
              <form onSubmit={goToLocation} id="reg-form" className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" id="business_name" required value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="e.g. Bella Salon" className="w-full bg-surface-container-low border border-surface-variant rounded-[14px] px-4 py-3 text-base text-on-background focus:border-primary-container focus:ring-4 focus:ring-primary-fixed-dim transition-all placeholder:text-on-surface-variant/50 outline-none" />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select id="business_category" value={businessCategory} onChange={(e) => setBusinessCategory(e.target.value)} required className="w-full bg-surface-container-low border border-surface-variant rounded-[14px] px-4 py-3 text-base text-on-background focus:border-primary-container focus:ring-4 focus:ring-primary-fixed-dim transition-all appearance-none pr-10 outline-none">
                ...
                <div>
                  ...
                  <input type="tel" id="contact_number" required value={contactNumber} onChange={(e) => setContactNumber(e.target.value)} placeholder="(555) 123-4567" className="w-full bg-surface-container-low border border-surface-variant rounded-[14px] px-4 py-3 text-base text-on-background focus:border-primary-container focus:ring-4 focus:ring-primary-fixed-dim transition-all placeholder:text-on-surface-variant/50 outline-none" />
                </div>
              </form>
            </motion.div>
          </>
          ...
          <>
            <div className="mb-6 text-center">
              <h2 className="text-2xl font-semibold text-on-background mb-2 flex items-center justify-center gap-2">
                <MapPin className="w-5 h-5 text-primary" /> Shop Location *
              </h2>
              <p className="text-base text-on-surface-variant">
                Apne shop ki exact location set karo — 'Use Current Location' ya map par pin drag karo. Location confirm hona zaroori hai.
              </p>
            </div>
            ...
            <motion.div
              ...
              transition={{ duration: 0.4, delay: 0.1 }}
            >
              <ShopLocationPicker onConfirm={setConfirmedLocation} confirmed={confirmedLocation} />
            </motion.div>
          </>
        )}
      </main>
      ...
      <div className="fixed bottom-0 left-0 w-full p-5 pb-[env(safe-area-inset-bottom,20px)] bg-surface/90 backdrop-blur-[20px] border-t border-surface-variant shadow-[0px_-4px_20px_rgba(0,0,0,0.03)] z-40">
        <div className="max-w-[600px] mx-auto w-full flex flex-col gap-4">
          {step === 1 && (
            <label className="flex items-start gap-3 cursor-pointer group">
              <div className="relative flex items-center justify-center mt-0.5">
                <input type="checkbox" checked={policyAgreed} onChange={(e) => setPolicyAgreed(e.target.checked)} className="peer h-5 w-5 cursor-pointer appearance-none rounded border border-outline-variant bg-surface transition-all checked:border-primary checked:bg-primary" />
          ...
          {step === 1 ? (
            ...
            >
              Next: Set Shop Location <ArrowRight className="w-5 h-5" />
            </button>
          ) : (
            <div className="flex gap-3">
              <button
                ...
                onClick={() => setStep(1)}
                disabled={loading}
                className="w-24 bg-surface-container-high text-on-surface rounded-[16px] py-4 text-sm font-semibold flex items-center justify-center gap-1 hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={loading || !confirmedLocation}
                className="flex-1 bg-primary-container text-white rounded-[16px] py-4 text-base font-semibold flex items-center justify-center gap-2 hover:opacity-90 active:scale-[0.98] transition-all shadow-sm disabled:opacity-50"
              >
                {loading ? 'Creating Account...' : 'Confirm & Create Account'}
                {!confirmedLocation && <span className="text-[10px] font-bold bg-white/20 px-2 py-0.5 rounded-full">Location required</span>}
              </button>
            </div>
          )}
        </div>
