...
export default function TopBar({ 
  ...
  return (
    <header className={`sticky top-0 w-full z-50 border-b flex justify-between items-center px-4 sm:px-5 min-h-16 pt-safe transition-colors ${
      transparent 
      ...
      <div className="flex items-center gap-2.5 sm:gap-3">
        {showBack ? (
          ...
          <button onClick={() => navigate('profile')} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full overflow-hidden border border-outline-variant bg-surface-container shrink-0">
            <img 
        ...
        <h1 className={`text-lg sm:text-xl font-semibold tracking-tight truncate max-w-[140px] sm:max-w-none ${showBack ? 'text-primary' : 'text-primary'}`}>
          {title}
        </h1>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {!showBack && (
          <button 
            ...
            className="flex items-center gap-1.5 text-primary bg-primary/10 hover:bg-primary/20 px-2.5 sm:px-3.5 py-1.5 rounded-full text-xs font-bold transition-all md:hidden cursor-pointer"
          >
            <Megaphone className="w-3.5 h-3.5" />
            <span className="hidden xs:inline">{t('marketing')}</span>
          </button>
        )}
        ...
        {showSettings ? (
          ...
          <button className="relative w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-full text-on-surface-variant hover:bg-surface-container transition-colors active:scale-95">
        ...
        {!showBack && (
          <div className="hidden md:flex gap-6 lg:gap-8 items-center ml-2">
...
/* ═══════════════════════════════════════════════════════════════════
   Website Builder TopBar (consolidated)
   ═══════════════════════════════════════════════════════════════════ */

/**
 * BuilderTopBar — toolbar for the Website Builder module (25-screen navigator,
 * module switcher, save status, wizard progress).
 *
 * Consolidated here from the former `src/website-builder/components/TopBar.tsx`
 * so the app has exactly ONE TopBar module. The default export of this file is
 * the main app-shell TopBar; the website-builder imports `BuilderTopBar`.
 */
import { Sparkles, CheckCircle2, AlertCircle, Users, LayoutDashboard, Loader2, ChevronDown, Calendar } from 'lucide-react';
...
export function BuilderTopBar({ step, activeModule, setActiveModule, saveStatus = 'saved', currentScreen, onNavigate }: Props) {
  ...
  const currentLabel = BUILDER_SCREENS.find(s => s.id === derivedScreen)?.label || `Screen ${derivedScreen}`;
              ...
              {['WIZARD (01-16)', 'STAFF MODULE', 'DASHBOARD (18-25)'].map(group => (
                <div key={group} className="p-2">
                  ...
                  <div className="space-y-0.5">
                    {BUILDER_SCREENS.filter(s => s.group === group).map(screen => (
                      <button
                        key={screen.id}
